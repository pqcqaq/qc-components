import { c as pe, b as U, O as he, E as Ge, _ as E, ac as Y, ad as _e, i as Me, ae as Pe, P as se, x as ze, H as Ke, A as We, u as Ue, V as Xe, y as Ye, af as qe, ag as Je, ah as Qe, a3 as Ne, D as fe, C as Ze, G as et } from "./styleChecker-DdDg1L8e.js";
...
import { u as at, L as it, T as st, c as ut, i as Le, a as Se, b as rt, f as ct, d as dt, t as ft, B as vt, e as mt, g as pt, h as ht } from "./index-CelQK0t5.js";
import { K as W } from "./KeyCode-n3eOOGSl.js";
import { p as bt } from "./CheckOutlined-glN3v3hv.js";
import { u as gt } from "./move-B6--odHQ.js";
import { o as be } from "./omit-CH0b8HlL.js";
import { u as Ee } from "./useMergedState-6HyZ0xMy.js";
import { a as yt, g as St } from "./statusUtils-BkxkBBZV.js";
function Ot() {
...
const Vt = Oe({
  ...
  setup(e, c) {
    ...
    function D(o) {
      ...
      return b ? H("div", U(U({
        "aria-label": typeof G == "string" && !P ? G : null
      }, B), {}, {
        key: o,
...
function At(e) {
  ...
  if (Y(c !== "tags" || X.every((M) => !M.disabled), "Please avoid setting option to disabled in tags mode since user can always type text as tag."), Y(c !== "combobox" || !C, "`combobox` mode not support `optionLabelProp`. Please set `value` on Option directly."), Y(c === "combobox" || !n, "`backfill` only works with `combobox` mode."), Y(c === "combobox" || !d, "`getInputElement` only work with `combobox` mode."), _e(c !== "combobox" || !d || !u || !v, "Customize `getInputElement` should customize clear and placeholder logic instead of configuring `allowClear` and `placeholder`."), y && !D && c !== "combobox" && c !== "tags" && Y(!1, "`onSearch` should work with `showSearch` instead of use alone."), _e(!p || O, "`defaultOpen` makes Select open without focus which means it will not close by click outside. You can set `autofocus` if needed."), I != null) {
    const M = Ce(I);
    Y(!F || M.every((V) => typeof V == "object" && ("key" in V || "value" in V)), "`value` should in shape of `{ value: string | number, label?: any }` when you set `labelInValue` to `true`"), Y(!x || Array.isArray(I), "`value` should be array when `mode` is `multiple` or `tags`");
  }
  if (l) {
    let M = null;
    l.some((V) => {
    ...
    }), M && Y(!1, `\`children\` should be \`Select.Option\` or \`Select.OptGroup\` instead of \`${M.displayName || M.name || M}\`.`), Y(w === void 0, "`inputValue` is deprecated, please use `searchValue` instead.");
  }
}
function ge(e, c) {
...
const kt = Oe({
  ...
  setup(e, c) {
    ...
    const B = Et(P, y, p, m, Q(e, "optionFilterProp")), G = S(() => e.mode !== "tags" || !p.value || B.value.some((t) => t[e.optionFilterProp || "value"] === p.value) ? B.value : [f(p.value), ...B.value]), ue = S(() => e.filterSort ? [...G.value].sort((t, a) => e.filterSort(t, a)) : G.value), z = S(() => dt(ue.value, {
      fieldNames: y.value,
...
const Tt = () => E(E({}, be($e(), ["inputIcon", "mode", "getInputElement", "getRawInputElement", "backfill"])), {
  ...
  setup(e, c) {
    ...
    const v = Te(), d = ze(), m = Ke.useInject(), y = S(() => yt(m.status, e.status)), p = () => {
      var h;
    ...
    } = Xe(w, C), P = S(() => _.value || X.value), B = Ye(), G = S(() => {
      var h;
      return (h = o.value) !== null && h !== void 0 ? h : B.value;
    }), [ue, z] = pt(w), R = S(() => M()), re = S(() => e.placement !== void 0 ? e.placement : C.value === "rtl" ? "bottomRight" : "bottomLeft"), L = S(() => qe(R.value, Je(re.value), e.transitionName)), Z = S(() => pe({
    ...
    }, St(w.value, y.value, m.hasFeedback), f.value, z.value)), ee = function() {

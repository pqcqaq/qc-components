import { g as pe, H as be, h as U, n as he, $ as ze, K as W, e as Ge, _ as E, al as Y, ah as _e, k as De, am as Pe, P as se, u as Ke, F as We, A as Ue, c as Xe, af as Ye, d as qe, aC as Je, ag as Qe, aU as Ze, C as Ne, s as fe, b as et, f as tt } from "./index-DQQY7Ife.js";
import { provide as nt, inject as lt, defineComponent as Oe, computed as S, reactive as ot, watch as ye, toRaw as Me, nextTick as Ae, createVNode as H, Fragment as at, shallowRef as ie, watchEffect as me, toRef as Q, ref as Te } from "vue";
import { u as it, L as st, T as ut, f as rt, i as Le, h as Se, b as ct, j as dt, k as ft, t as vt, B as mt, d as pt, e as ht, g as bt } from "./index-ReaMVeJG.js";
import { u as gt } from "./move-DAHHGcHZ.js";
import { u as Ee } from "./useMergedState-6HyZ0xMy.js";
import { g as yt, a as St } from "./statusUtils-B6e7x_W2.js";
function Ot() {
...
const Vt = Oe({
  ...
  setup(e, c) {
    ...
    const n = it(), u = Ct(), v = S(() => `${n.prefixCls}-item`), d = gt(() => u.flattenOptions, [() => n.open, () => u.flattenOptions], (o) => o[0]), m = rt(), y = (o) => {
      o.preventDefault();
    }, p = (o) => {
    ...
    ye([() => n.open, () => n.searchValue], () => {
      if (!n.multiple && n.open && u.rawValues.size === 1) {
        const o = Array.from(u.rawValues)[0], b = Me(d.value).findIndex((_) => {
          let {
            data: f
          } = _;
...
function Pt(e, c, g) {
  ...
  return ye([e, c], () => {
    e.value ? v.value = Me(e.value).slice() : v.value = we(c.value);
  }, {
    immediate: !0,
    deep: !0
  }), me(() => {
...
function At(e) {
  const {
    ...
    inputValue: w,
    optionLabelProp: C
  } = e, x = Le(c), k = m !== void 0 ? m : x || c === "combobox", X = g || we(l);
  if (Y(c !== "tags" || X.every((D) => !D.disabled), "Please avoid setting option to disabled in tags mode since user can always type text as tag."), Y(c !== "combobox" || !C, "`combobox` mode not support `optionLabelProp`. Please set `value` on Option directly."), Y(c === "combobox" || !n, "`backfill` only works with `combobox` mode."), Y(c === "combobox" || !d, "`getInputElement` only work with `combobox` mode."), _e(c !== "combobox" || !d || !u || !v, "Customize `getInputElement` should customize clear and placeholder logic instead of configuring `allowClear` and `placeholder`."), y && !k && c !== "combobox" && c !== "tags" && Y(!1, "`onSearch` should work with `showSearch` instead of use alone."), _e(!p || O, "`defaultOpen` makes Select open without focus which means it will not close by click outside. You can set `autofocus` if needed."), I != null) {
    const D = Ce(I);
    Y(!F || D.every((V) => typeof V == "object" && ("key" in V || "value" in V)), "`value` should in shape of `{ value: string | number, label?: any }` when you set `labelInValue` to `true`"), Y(!x || Array.isArray(I), "`value` should be array when `mode` is `multiple` or `tags`");
  }
  if (l) {
    let D = null;
    l.some((V) => {
      ...
      return b.isSelectOption ? !1 : b.isSelectOptGroup ? !(((o = V.children) === null || o === void 0 ? void 0 : o.default()) || []).every((P) => !he(P) || !V.type || P.type.isSelectOption ? !0 : (D = P.type, !1)) : (D = b, !0);
    }), D && Y(!1, `\`children\` should be \`Select.Option\` or \`Select.OptGroup\` instead of \`${D.displayName || D.name || D}\`.`), Y(w === void 0, "`inputValue` is deprecated, please use `searchValue` instead.");
  }
}
function ge(e, c) {
...
const Et = (e, c, g, l, n) => S(() => {
  ...
  const {
    ...
    label: y,
    value: p
  } = c.value, O = [], F = typeof d == "function", I = u.toUpperCase(), w = F ? d : (x, k) => v ? ge(k[v], I) : k[m] ? ge(k[y !== "children" ? y : "label"], I) : ge(k[p], I), C = F ? (x) => Se(x) : (x) => x;
  return e.value.forEach((x) => {
    if (x[m]) {
      if (w(u, C(x)))
        O.push(x);
      else {
        const X = x[m].filter((D) => w(u, C(D)));
...
const Mt = Oe({
  ...
  setup(e, c) {
    ...
    const u = ct(Q(e, "id")), v = S(() => Le(e.mode)), d = S(() => !!(!e.options && e.children)), m = S(() => e.filterOption === void 0 && e.mode === "combobox" ? !1 : e.filterOption), y = S(() => dt(e.fieldNames, d.value)), [p, O] = Ee("", {
      ...
      var r, s;
      let A, J, j, ae;
      Dt(i) ? A = i : (j = i.key, J = i.label, A = (r = i.value) !== null && r !== void 0 ? r : j);
    ...
    const B = Et(P, y, p, m, Q(e, "optionFilterProp")), z = S(() => e.mode !== "tags" || !p.value || B.value.some((t) => t[e.optionFilterProp || "value"] === p.value) ? B.value : [f(p.value), ...B.value]), ue = S(() => e.filterSort ? [...z.value].sort((t, a) => e.filterSort(t, a)) : z.value), G = S(() => ft(ue.value, {
      fieldNames: y.value,
...
const Tt = () => E(E({}, be($e(), ["inputIcon", "mode", "getInputElement", "getRawInputElement", "backfill"])), {
  ...
  status: fe(),
  "onUpdate:value": tt()
}), ke = "SECRET_COMBOBOX_MODE_DO_NOT_USE", q = Oe({
  compatConfig: {
    MODE: 3
  },
  name: "ASelect",
  ...
  setup(e, c) {
    ...
    const v = Te(), d = Ke(), m = We.useInject(), y = S(() => yt(m.status, e.status)), p = () => {
      var h;
      ...
      var M;
      (M = v.value) === null || M === void 0 || M.scrollTo(h);
    }, I = S(() => {
      const {
        mode: h
      } = e;
    ...
    process.env.NODE_ENV !== "production" && Ue(!e.dropdownClassName, "Select", "`dropdownClassName` is deprecated. Please use `popupClassName` instead.");
    const {
      prefixCls: w,
      direction: C,
      configProvider: x,
    ...
    } = Ye(w, C), P = S(() => _.value || X.value), B = qe(), z = S(() => {
      var h;
      return (h = o.value) !== null && h !== void 0 ? h : B.value;
    }), [ue, G] = ht(w), R = S(() => D()), re = S(() => e.placement !== void 0 ? e.placement : C.value === "rtl" ? "bottomRight" : "bottomLeft"), L = S(() => Je(R.value, Qe(re.value), e.transitionName)), Z = S(() => pe({
      ...
      for (var h = arguments.length, M = new Array(h), T = 0; T < h; T++)
        M[T] = arguments[T];
      l("update:value", M[0]), l("change", ...M), d.onFieldChange();
    }, ce = (h) => {
    ...
    return () => {
      ...
      let N;
      te !== void 0 ? N = te : n.notFoundContent ? N = n.notFoundContent() : I.value === "combobox" ? N = null : N = (k == null ? void 0 : k("Select")) || H(Ze, {
      ...
      return ue(H(Mt, U(U(U({
        ref: v,
        virtual: i,
        dropdownMatchSelectWidth: r
      }, xe), g), {}, {
        showSearch: (M = e.showSearch) !== null && M !== void 0 ? M : (T = b == null ? void 0 : b.value) === null || T === void 0 ? void 0 : T.showSearch,
        placeholder: A,
        listHeight: ne,
        listItemHeight: de,
        mode: I.value,
...
q.install = function(e) {
  return e.component(q.name, q), e.component(q.Option.displayName, q.Option), e.component(q.OptGroup.displayName, q.OptGroup), e;
};
const zt = q.Option;
q.OptGroup;
export {
